			  ...
			  union bpf_attr __user *uattr)
{
	...
	if (ctx) {
		...
		if (ctx->data_meta || ctx->data_end != size ||
			...
			goto free_ctx;
		if (xdp_metalen_invalid(ctx->data) ||
		    ctx->data > headroom - sizeof(struct xdp_frame))
			goto free_ctx;
	...
	/*
	 * xdp_page_head takes up space in each live page. Account it in the
	 * packet length checks below, but not in the metadata size check above.
	 */
	if (do_live)
